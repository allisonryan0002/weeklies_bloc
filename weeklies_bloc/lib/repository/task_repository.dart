...
class TaskRepository {
  ...
  Future<List<Task>> loadTasks() async {
    final fileContents = await FileClient.read('myTasks.json');
  ...
  Future<File> saveTasks(List<Task> tasks) async {
    ...
    return FileClient.write('myTasks.json', contents);
  }

  Future<SortType> loadSort() async {
    final fileContents = await FileClient.read('mySort.json');
    final jsonContent = JsonDecoder().convert(fileContents);
  ...
  Future<File> saveSort(SortType sort) async {
    ...
    return FileClient.write('mySort.json', contents);
  }
}
