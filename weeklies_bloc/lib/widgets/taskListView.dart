...
class _TaskListViewState extends State<TaskListView> {
  ...
  changePriorityWindow(BuildContext priorityContext, Task item) {
    updatePriority(ItemPriority p) {
      //TODO: Might need new and old task - if you don't, remove item input
      BlocProvider.of<TasksBloc>(priorityContext).add(
          TaskUpdated(Task(item.key, item.globalKey, item.task, p, item.time)));
  ...
  changeDayWindow(BuildContext dayContext, Task item) {
    updateDay(int time) {
      //TODO: Might need to send in new and old to update - see orignal taskList
      BlocProvider.of<TasksBloc>(dayContext).add(TaskUpdated(
          Task(item.key, item.globalKey, item.task, item.priority, time)));
  ...
  Widget build(BuildContext context) {
    //TODO: Implement loading screen and other sorting screen
    //if (Provider.of<TaskList>(context).prioritySort) {
    return BlocProvider(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (index == state.tasks.length) {
                      ...
                      var taskItem = state.tasks[index];
    ...
    // } else {
    //   List<TaskItem> tasks = Provider.of<TaskList>(context).tasks;
    ...
    //                     return getTaskDismissiblePanel(taskItem, taskIndex);
    //                   },
    //                 ),
    //               ],
    //             ),
